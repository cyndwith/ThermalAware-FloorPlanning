...
struct hspBlock *hotspot(int argc, char **argv) //,struct hspBlock *hotspotData)
{
    struct hspBlock *hotspotData=(struct hspBlock *)malloc(NUM_BLOCKS*sizeof(struct hspBlock));
	...
	// Return block and its average steady state temperature
	//printf("\nDebug Print:\n");
	flp = (model->block)->flp;
    for(i=0;i<30;i++)
           // printf("\t%d\t%.2f\n",i,steady_temp[i]);
    for (i=0; i < 30; i++){
        strncpy(hotspotData[i].name,flp->units[i].name, sizeof(flp->units[i].name));
        hotspotData[i].steady_temp = steady_temp[i];
        //fprintf(stdout, "%d\t%s\t%.2f\n", i,hotspotData[i].name, hotspotData[i].steady_temp);
    }
    //printf("Clean Up!\n");
